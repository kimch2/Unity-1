...
public class MoveInput : IInputControl
{
    ...
    { 
        ...
        if (camera.transform.position.x > FreeLookCameraComponent.ClampMaxX || camera.transform.position.x < FreeLookCameraComponent.ClampMinX)
        {
            target.transform.position = new Vector3(previewargetPos.x, target.transform.position.y, target.transform.position.z);
        }

        if (camera.transform.position.z > FreeLookCameraComponent.ClampMaxZ || camera.transform.position.z < FreeLookCameraComponent.ClampMinZ)
        {
            target.transform.position = new Vector3(target.transform.position.x, target.transform.position.y, previewargetPos.z);
        }
