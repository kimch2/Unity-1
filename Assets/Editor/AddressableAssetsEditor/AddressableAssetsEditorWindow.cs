...
namespace ETEditor
{
    public class AddressableAssetsEditorWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            if (!File.Exists($"../Release/{UnityEditor.EditorUserBuildSettings.activeBuildTarget}/ChangeLog.txt"))
            {
                using (var fs = File.Create($"../Release/{UnityEditor.EditorUserBuildSettings.activeBuildTarget}/ChangeLog.txt")) 
                {
        ...
        private void OnGUI()
        {
            ...
            using (EditorGUILayout.VerticalScope vertical = new EditorGUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.MinHeight(this.position.height - 335)))
            {
                ...
                using (EditorGUILayout.ScrollViewScope scroll = new EditorGUILayout.ScrollViewScope(this.scrollPosition, GUILayout.ExpandWidth(true), GUILayout.MinHeight(this.position.height - 360)))
                {
            ...
            if (GUI.Button(new Rect(2, this.position.height - 42, this.position.width - 4, 40), "打包"))
            {
